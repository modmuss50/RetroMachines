...
pub fn get_mbc(data: Vec<u8>, savepath: Option<path::PathBuf>, skip_checksum: bool) -> StrResult<Box<dyn MBC+'static>> {
    ...
    match data[0x147] {
        ...
        0x01 ..= 0x03 => mbc1::MBC1::new(data, savepath).map(|v| Box::new(v) as Box<dyn MBC>),
        0x05 ..= 0x06 => mbc2::MBC2::new(data, savepath).map(|v| Box::new(v) as Box<dyn MBC>),
        0x0F ..= 0x13 => mbc3::MBC3::new(data, savepath).map(|v| Box::new(v) as Box<dyn MBC>),
        0x19 ..= 0x1E => mbc5::MBC5::new(data, savepath).map(|v| Box::new(v) as Box<dyn MBC>),
